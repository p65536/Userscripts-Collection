...
(function () {
  ...
  const CONSTANTS = {
    ...
    PROCESSED_CLASS: `${APPID}-processed`,
  };
  ...
  const SITE_STYLES = {
    google: {
      ...
      accent: 'rgb(66 133 244)', // Google Blue
      text_secondary: 'var(--secondary-text-color, GrayText)',

      // Variable mapping specific to Button UI
      vars: {
        [`--${APPID}-bg`]: 'var(--background-color, Canvas)',
        [`--${APPID}-text`]: 'var(--primary-text-color, CanvasText)',
        [`--${APPID}-border`]: 'var(--border-color, ButtonBorder)',
        // Hover bg: Solid color (Google Light Gray) instead of transparent
        [`--${APPID}-hover-bg`]: 'var(--hover-bg-color, rgb(241 243 244))',
        [`--${APPID}-accent`]: 'rgb(66 133 244)',
        // Alert Colors (Light Mode Default)
        [`--${APPID}-unsafe`]: 'rgb(217 48 37)',
        [`--${APPID}-noref`]: 'rgb(26 13 171)', // Google Link Blue
      },
...
--${APPID}-hover-bg: rgb(48 49 52);
--${APPID}-unsafe: rgb(224 96 85); /* Bright red for dark mode */
--${APPID}-noref: rgb(138 180 248);  /* Bright blue for dark mode */
}
}
`,
    ...
    bing: {
      ...
      accent: 'rgb(0 120 212)', // Bing Blue
      text_secondary: 'var(--c-t-2, GrayText)',

      // Variable mapping specific to Button UI
      vars: {
        [`--${APPID}-bg`]: 'var(--c-w-1)',
        [`--${APPID}-text`]: 'var(--c-t-1)',
        [`--${APPID}-border`]: 'var(--c-s-1)',
        [`--${APPID}-hover-bg`]: 'var(--c-s-2)',
        [`--${APPID}-accent`]: 'rgb(0 120 212)',
        [`--${APPID}-unsafe`]: 'rgb(217 48 37)',
        [`--${APPID}-noref`]: 'var(--c-h-1)', // Bing link color
      },
...
`,
    ...
    duckduckgo: {
      ...
      bg: 'var(--color-bg-main, rgb(255 255 255))',
      text: 'var(--color-text-primary, rgb(51 51 51))',
      border: 'var(--color-border-main, rgb(204 204 204))',
      header_bg: 'var(--color-bg-main, rgb(255 255 255))',
      btn_bg: 'var(--color-bg-main, rgb(255 255 255))',
      btn_text: 'var(--color-text-primary, rgb(51 51 51))',
      btn_border: 'var(--color-border-main, rgb(204 204 204))',
      btn_hover_bg: 'var(--color-bg-dim, rgb(240 240 240))',
      input_bg: 'var(--color-bg-input, rgb(255 255 255))',
      input_text: 'var(--color-text-primary, rgb(51 51 51))',
      input_border: 'var(--color-border-main, rgb(204 204 204))',
      accent: 'rgb(222 88 51)', // DDG Orange/Red
      text_secondary: 'var(--color-text-secondary, rgb(102 102 102))',

      // Variable mapping specific to Button UI
      vars: {
        [`--${APPID}-bg`]: 'var(--color-bg-main, rgb(255 255 255))',
        [`--${APPID}-text`]: 'var(--color-text-primary, rgb(51 51 51))',
        [`--${APPID}-border`]: 'var(--color-border-main, rgb(0 0 0 / 0.1))',
        [`--${APPID}-hover-bg`]: 'var(--color-bg-dim, rgb(240 240 240))',
        [`--${APPID}-accent`]: 'rgb(222 88 51)',
        [`--${APPID}-unsafe`]: 'rgb(217 48 37)',
        [`--${APPID}-noref`]: 'rgb(64 150 255)',
      },
      css_overrides: '',
...
height: 32px;
border-radius: 50%;
background-color: var(--${APPID}-bg, rgb(255 255 255));
border: 1px solid var(--${APPID}-border, rgb(0 0 0 / 0.1));
...
color: var(--${APPID}-text, rgb(95 99 104));
transition: transform 0.1s, background-color 0.1s, opacity 0.2s ease-in-out;
z-index: 10;
}
.${APPID}-icon-btn:hover {
background-color: var(--${APPID}-hover-bg, rgb(241 243 244));
color: var(--${APPID}-text, rgb(32 33 36));
transform: scale(1.1);
...
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.ORIGIN}"] .${APPID}-icon-btn { color: var(--${APPID}-text) !important; }
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.ORIGIN}"] .${APPID}-icon-btn:hover { color: var(--${APPID}-text) !important; }

/* Unsafe URL - Red (Alert) */
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.UNSAFE_URL}"] .${APPID}-icon-btn { color: var(--${APPID}-unsafe) !important; }
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.UNSAFE_URL}"] .${APPID}-icon-btn:hover { color: var(--${APPID}-unsafe) !important; }

/* No Referrer - Blue (Custom/Link Color) */
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.NO_REFERRER}"] .${APPID}-icon-btn { color: var(--${APPID}-noref) !important; }
body[data-${APPID}-referrer-policy="${CONSTANTS.REFERRER_POLICY.NO_REFERRER}"] .${APPID}-icon-btn:hover { color: var(--${APPID}-noref) !important; }
...
.${APPID}-toast-info { background-color: rgb(51 51 51); }
.${APPID}-toast-warn { background-color: rgb(251 188 4); color: rgb(32 33 36); }
.${APPID}-toast-error { background-color: rgb(217 48 37); }
...
`,
  ...
  class Logger {
    ...
    static styles = {
      ...
      RED: 'background: rgb(220 53 69);',
      YELLOW: 'background: rgb(255 193 7); color: black;',
      GREEN: 'background: rgb(40 167 69);',
      BLUE: 'background: rgb(0 123 255);',
      GRAY: 'background: rgb(108 117 125);',
      ORANGE: 'background: rgb(253 126 20);',
      PINK: 'background: rgb(232 62 140);',
      PURPLE: 'background: rgb(111 66 193);',
      CYAN: 'background: rgb(23 162 184); color: black;',
      TEAL: 'background: rgb(32 201 151); color: black;',
    };
  ...
  class ConfigManager {
    ...
    async load() {
      ...
      EventBus.setDebug(this.config.developer.logger_level === 'debug');
    }
  ...
  class SettingsModal extends BaseManager {
    /**
    ...
    open() {
      ...
      const createStrategyDesc = (val) => {
        ...
        return h(`div#${APPID}-strategy-desc`, { style: { marginTop: '4px', color: 'rgb(154 160 166)' } }, text);
      };
        ...
        [
          h(`div.${APPID}-modal-box`, [
            ...
            h(`div.${APPID}-modal-content`, [
              ...
              h('hr', { style: { border: '0', borderTop: '1px solid rgb(95 99 104)', margin: '8px 0 16px 0' } }),
            ...
            h(`div.${APPID}-modal-footer`, [
              ...
              // Right: Actions
              h(`div.${APPID}-footer-actions`, [h(`button.${APPID}-ui-btn.${APPID}-btn-secondary`, { onclick: () => this.close() }, 'Cancel'), h(`button.${APPID}-ui-btn.${APPID}-btn-primary`, { onclick: () => this.save() }, 'Save')]),
            ]),
          ]),
        ]
      );
      ...
      this.addDisposable(() => document.removeEventListener('keydown', this._boundHandleKeyDown));
    }
...
.${APPID}-form-label {
display: block; margin-bottom: 8px; font-weight: 600; color: rgb(232 234 237);
}
...
.${APPID}-form-desc {
color: rgb(154 160 166); margin-bottom: 6px; line-height: 1.4;
}
.${APPID}-form-input {
width: 100%; padding: 6px 8px;
background: rgb(48 49 52); border: 1px solid rgb(95 99 104); border-radius: 4px;
color: rgb(232 234 237); box-sizing: border-box;
}
.${APPID}-form-input:focus {
border-color: rgb(138 180 248); outline: 1px solid rgb(138 180 248);
}
.${APPID}-form-select {
width: 100%; padding: 6px 8px;
background: rgb(48 49 52); border: 1px solid rgb(95 99 104); border-radius: 4px;
...
display: flex; align-items: center; gap: 8px; color: rgb(232 234 237);
}
.${APPID}-ui-btn {
padding: 6px 16px; border-radius: 4px; border: 1px solid rgb(95 99 104);
cursor: pointer; font-size: 13px; font-weight: 500;
transition: background 0.1s;
white-space: nowrap;
}
.${APPID}-btn-primary {
background: rgb(138 180 248); color: rgb(32 33 36); border: 1px solid rgb(138 180 248);
}
.${APPID}-btn-primary:hover {
opacity: 0.9;
}
.${APPID}-btn-secondary {
background: rgb(48 49 52); color: rgb(232 234 237);
}
...
`;
  ...
  class UIManager extends BaseManager {
    ...
    constructor(configManager) {
      ...
      this.registerPlatformListener(EVENTS.CONFIG_UPDATED, () => this.updateStyles());
    }
    ...
    init(platformStyles) {
      ...
      // Clear all active toasts instantly when the page visibility shifts to background
      const handleVisibilityChange = () => {
        if (document.visibilityState === 'hidden' && this.toastContainer) {
          this.toastContainer.textContent = '';
        }
      };
      document.addEventListener('visibilitychange', handleVisibilityChange);
      this.addDisposable(() => document.removeEventListener('visibilitychange', handleVisibilityChange));
    }
    ...
    _injectStyles(platformStyles) {
      ...
      if (style) {
        // Update existing style content for platform switching
        style.textContent = cssContent;
        return;
      }

      style = h('style', { id }, cssContent);
    ...
    _createToastContainer() {
      // Check if the container already exists to prevent DOM leaks
      const existingContainer = document.querySelector(`.${APPID}-toast-container`);
    ...
    attachButtons(sentinelElement, targetContainer) {
      ...
      const processedClass = CONSTANTS.PROCESSED_CLASS;
    ...
    async _handleImageClick(e) {
      ...
      try {
        ...
        if (targetUrl.startsWith('http://')) {
          // HTTP scheme detected. Forcing Direct mode to avoid Mixed Content blocks.
          Logger.info('DECISION', LOG_STYLES.PURPLE, 'HTTP scheme detected. Forcing Direct mode.');
          useBlob = false;
        } else {
        ...
        if (useBlob) {
          ...
          Logger.info('OPENING', LOG_STYLES.GREEN, `Opening Direct URL: "${targetUrl}"`);

          if (e.button === 1) {
            // Middle Click: Synthetic auxclick events do not trigger native tab navigation on anchors.
            // Fallback to GM.openInTab to reliably open the link in the background.
            GM.openInTab(targetUrl, { active: false, insert: false });
          } else {
            // Left Click or Ctrl/Meta + Click (button === 0): Use dynamic anchor to respect referrer policy settings.
            const a = document.createElement('a');
            a.href = targetUrl;
            a.target = '_blank';

            const currentPolicy = config.common.referrerPolicy;
            a.setAttribute('referrerpolicy', currentPolicy);
            const relValue = currentPolicy === CONSTANTS.REFERRER_POLICY.NO_REFERRER ? 'noopener noreferrer' : 'noopener';
            a.setAttribute('rel', relValue);
            a.style.display = 'none';
            document.body.appendChild(a);

            // Dispatch a cloned MouseEvent to preserve background/foreground preferences for click events.
        ...
        if (fallbackUrl) {
          ...
          if (e.button === 1) {
            ...
            GM.openInTab(fallbackUrl, { active: false, insert: false });
          } else {
            // Left Click or Ctrl/Meta + Click fallback
            const a = document.createElement('a');
            a.href = fallbackUrl;
            a.target = '_blank';
   ...
   */
  class BaseAdapter extends BaseManager {
    /**
    ...
    /**
     * Extracts the high-resolution image URL and the host page URL from the result element.
     ...
     */
    extractUrls(element) {
      throw new Error('extractUrls() must be implemented by subclasses.');
    }
  ...
  class BingAdapter extends BaseAdapter {
    ...
    static isApplicable() {
      ...
      return (
                ...
                (window.location.pathname.startsWith('/images/search') || window.location.pathname.startsWith('/images/vnext/search'))
            );
    }
  ...
  class DuckDuckGoAdapter extends BaseAdapter {
    ...
    getSentinelSelector() {
      ...
      return `figure:not(.${CONSTANTS.PROCESSED_CLASS})`;
    }
    ...
    async fetchOriginalImageUrl(element) {
      return new Promise((resolve) => {
        ...
        const trigger = element.querySelector('img');
        if (!trigger) {
        ...
        let pollTimeoutId = null; // Holds the polling timeout ID
        ...
        // Manage active fetch instance via resource manager
        this.manageResource(CONSTANTS.RESOURCE_KEYS.ACTIVE_FETCH, () => {
          // Guard against multiple resolutions when manager is destroyed
          if (!isResolved) {
            isResolved = true;
            finalize();
            resolve(null);
        ...
        const checkPanel = () => {
          ...
          if (attempts >= MAX_ATTEMPTS) {
            ...
            if (!isResolved) {
              isResolved = true;
              finalize();
              this.manageResource(CONSTANTS.RESOURCE_KEYS.ACTIVE_FETCH, null);
              resolve(null);
            }
          } else {
            pollTimeoutId = setTimeout(checkPanel, CONSTANTS.TIMEOUTS.DOM_POLLING);
          }
        };
        ...
        const processPanel = (aside, container) => {
          ...
          // Step 1: Try using the class name approach
          let closeBtn = aside.querySelector('.ddgsi-close');
          if (closeBtn) {
            closeBtn = closeBtn.closest('button') || closeBtn;
          }

          // Step 2: Fallback to the last button inside the aside element if class name approach fails
          if (!closeBtn) {
            const buttons = aside.querySelectorAll('button');
            if (buttons.length > 0) {
              closeBtn = buttons[buttons.length - 1];
            }
          }

          // Step 3: Click the resolved close button or fallback to sending the Escape key
          if (closeBtn) {
            closeBtn.click();
          } else {
  ...
  class GoogleAdapter extends BaseAdapter {
    ...
    async fetchOriginalImageUrl(element) {
      ...
      if (element.tagName === 'A' && element.href.includes('/imgres')) {
        try {
          const params = new URLSearchParams(new URL(element.href).search);
      ...
      return new Promise((resolve) => {
        ...
        const lpage = element.dataset.lpage;

        // Identify the trigger button (div[role="button"]) inside the sentinel
        const trigger = element.querySelector('div[role="button"]');
        if (!trigger) {
        ...
        const nonce = document.querySelector('script[nonce]')?.nonce;
        if (nonce) stealthStyle.setAttribute('nonce', nonce);
  ...
  class AppController extends BaseManager {
    ...
    async init() {
      ...
      const subscriberKey = createSubscriberKey(CONSTANTS.NAV_PURPOSE.MAIN);
      const unsubscribe = this.navMonitor.on(subscriberKey, onNavStart, onNavSettled, { trackHash: false });
      this.addDisposable(unsubscribe);

      this.registerPlatformListener(EVENTS.NAVIGATION, this._reconcileAdapter.bind(this));
    ...
    _forceRescan() {
      ...
      const processedElements = document.querySelectorAll(`.${CONSTANTS.PROCESSED_CLASS}`);
      processedElements.forEach((el) => el.classList.remove(CONSTANTS.PROCESSED_CLASS));
    ...
    async _mountAdapter(AdapterClass) {
      ...
      // Instantiate Adapter using factory manager
      this.adapter = this.manageFactory(CONSTANTS.RESOURCE_KEYS.ADAPTER, () => new AdapterClass(this.uiManager));
      ...
      if (selector) {
        ...
        this.adapter.addDisposable(() => this.sentinel.off(selector, this.boundResultHandler));
      } else {
    ...
    _unmountAdapter() {
      ...
      // Unmount via BaseManager resource management
      this.manageResource(CONSTANTS.RESOURCE_KEYS.ADAPTER, null);
    ...
    _openSettings() {
      ...
      // Manage settings modal instance via factory manager
      const modal = this.manageFactory(CONSTANTS.RESOURCE_KEYS.SETTINGS_MODAL, () => new SettingsModal(this.configManager, styles));
      if (modal) {
        modal.open();
      }
    }
